import avatar1 from "../../assets/images/data/single-product-comments/1.jpg";
import avatar2 from "../../assets/images/data/single-product-comments/2.jpg";
import avatar3 from "../../assets/images/data/single-product-comments/3.jpg";
import avatar4 from "../../assets/images/data/single-product-comments/4.jpg";

const SingleProductCommentsData = [
  {
    avatar: avatar1,
    name: "نیکا شاکرمی",
    date: "۱۴۰۲/۰۵/۱۲",
    rating: 5,
    dislikes: 2,
    likes: 24,
    comment: "خوب و کارآمد مثل بقیه باتری‌های وارتا، من این مدل رو برای مولتی متر تهیه کردم. علیرغم اینکه تاریخ تولید و انقضا نداشت امیدوارم دوام خوبی داشته باشه.",
  },
  {
    avatar: avatar2,
    name: "سارینا اسماعیل زاده",
    date: "۱۴۰۲/۰۵/۱۲",
    rating: 5,
    dislikes: 0,
    likes: 100,
    comment: "خیلی چیز خوبیه واقعا خیلی قویه و عمرش خیلی طولانیه من الانم استفاده میکنم بازم خیلی خوبه",
  },
  {
    avatar: avatar3,
    name: "کیان پیرفلک",
    date: "۱۴۰۲/۰۵/۱۲",
    rating: 4,
    dislikes: 5,
    likes: 42,
    comment: "تازه گرفتیم مرور زمان کیفیت رو نشون میده",
  },
  {
    avatar: avatar4,
    name: "مهسا امینی",
    date: "۱۴۰۲/۰۵/۱۲",
    rating: 1,
    dislikes: 28,
    comment: "خییییییلی بی کیفیت بود پشیمون شدم از سفارشم",
    likes: 8,
    AdminReply: {
      date: "۱۴۰۲/۰۵/۱۲",
      comment: "سلام دوست عزیز ، امیدوارم در سفارش های بعدیتون تجربه بهتری داشته باشید.",
    },
  },
];

export default SingleProductCommentsData;
