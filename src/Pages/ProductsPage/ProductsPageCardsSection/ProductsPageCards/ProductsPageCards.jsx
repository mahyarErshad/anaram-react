...
function ProductsPageCards({ discountCard, outOfStock }) {
  ...
  return (
    <figure className={`${styles.card} ${discountCard && styles.after} ${outOfStock && "grayscale"} pt-3 pb-[0.875rem] max-sm:w-full max-sm:max-w-[25rem] sm:max-w-[15.75rem] relative px-3 rounded-lg bg-white border border-NeutralN30 grid sm:grid-cols-1 cursor-pointer transition-all duration-300`}>
      <ProductCardRatingSection position="top-3 left-2" rating={4.5} />
      <div className="flex-center h-full sm:mb-4 max-sm:col-span-1 max-sm:row-start-1 max-sm:row-end-5">
        <img className="sm:mt-5 transition-all duration-300" src={batteryImage} alt="battery" />
        {discountSection}
      </div>
